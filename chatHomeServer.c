...
int groupChat(int sockfd, int groupname, int name)
{
    ...
    /* 新建树的列表 群聊的列表  */
    balanceBinarySearchTreeInit(&AVLTree, communicate_handler, printFunc);

    int size = 0;
    int len = balanceBinarySearchTreeGetNodeSize(&AVLTree, size);
    ...
    {
        /* 遍历好友表 */
        int ret = strncmp(name, buf, strlen(name));
        if (ret == 0)
        {
            balanceBinarySearchTreeInsert(AVLTree, name);
        }
    }
    /* 给这个列表里面的所有人发信息 */
...
void *communicate_handler(void *arg)
{
    ...
    else
    {
        ...
        else if (json_object_get_int(json_object_object_get(parseObj, "choices")) == LOGIN)
        {
            ...
            {
                /*有重复*/
                strncpy(sendBuffer, "登录成功,开启你的聊天之旅吧！\n", sizeof(sendBuffer) - 1);
                int retwrite = write(fdset->acceptfd, sendBuffer, sizeof(sendBuffer) - 1);
                if (retwrite == -1)
                {
                    perror("write error");
            ...
            {
                /*无重复*/
                strncpy(sendBuffer, "账户不正确 请重新输入 \n", sizeof(sendBuffer) - 1);
                int retwrite = write(fdset->acceptfd, sendBuffer, sizeof(sendBuffer) - 1);
                if (retwrite == -1)
                {
                    perror("write error");
